...
var model = {
    ...
    fireBotNew: function(newRand, choice) {
        ...
        console.log(rand);
        console.log('Локация выстрела ' + model.shootsLoc[rand]);
        ...
        for (var i = 0; i < player.shipNum; i++){
            ...
            if (index >= 0){
                ...
                model.shoots++;
                console.log(new Date() + 'Попал бот ! ' + model.shootsLoc[rand]);
        ...
        console.log(new Date() + 'Промах бота ! ' + model.shootsLoc[rand]);
    ...
    secondShoot: function(rand, shoot, select){
        console.log('----- Начало повторного выстрела -----');
        var newRand, choice;
        ...
        console.log('point ' + newRand);
        console.log('index ' + secondShoot);
        console.log('выстрел ' + model.shootsLoc[secondShoot]);
        //model.again = false;
        console.log('----- Конец повторного выстрела -----');
...
// цикл начинать не с 0, а с 10
    ...
    posShipPl: function (){
        ...
        for (let i = 0; i < player.shipNum; i++){
            do {
                locations = this.generateShipPl(player.plShips[i].size)
...
// 2 одинаковых метода
    // начать вносить локации в резерв
    ...
    generateShip: function (shipSize) {
        var row,col;
        var pos = Math.floor(Math.random() * 2);
        if (pos === 1) { // horizontal
        ...
        for (var k = 0; k < 3; k++){
            if (pos === 1){
                ...
                reserved.push((rowRes + k).toString() + (col + shipSize));
            } else{
                reserved.push(rowRes.toString() + (colRes + k).toString());
                reserved.push((row + shipSize).toString() + (colRes + k).toString());
            }
        }
        return {
